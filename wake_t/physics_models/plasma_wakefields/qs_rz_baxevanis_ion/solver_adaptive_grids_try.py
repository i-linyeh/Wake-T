...
@njit_serial
def evolve_window_inplace_AG_build(
    pp_serialized_list,
    n_xi,
    n_r,
    dxi,
...
def build_pp_cache_at_kp1(
    ...
    if has_beam_source:
        bunch_source_arrays_arg = tuple(bunch_source_arrays)
        bunch_source_xi_indices_arg = tuple(bunch_source_xi_indices)
        bunch_source_metadata_arg = tuple(bunch_source_metadata)
    else:
        # These placeholders must match the REAL expected types.
        bunch_source_arrays_arg = (np.zeros((1, 1), dtype=np.float64),)
        bunch_source_xi_indices_arg = (np.zeros(1, dtype=np.int64),)
        bunch_source_metadata_arg = (np.zeros(3, dtype=np.int64),)
    ...
    evolve_window_inplace_AG_build(
        pp_cache,
        n_xi,
        n_r,
        dxi,
...
#        tuple(bunch_source_metadata),
        bunch_source_arrays_arg,
        bunch_source_xi_indices_arg,
        bunch_source_metadata_arg,
        max_gamma,
        ag_i_grid,
        ag_r_grid_n,
        ag_psi_grid,
