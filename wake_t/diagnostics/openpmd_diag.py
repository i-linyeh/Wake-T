...
class OpenPMDDiagnostics:
    ...
    def __init__(self, write_dir: Optional[str] = None) -> None:
    ...
    def _write_species(self, it, species_data):
        ...
        for component in components:
            if component in species_data:
                ...
                pos_off = species_data["z_off"] if component == "z" else 0.0
        ...
        if position_record:
            ...
            particles["position"].set_attribute("weightingPower", 0.0)
            particles["positionOffset"].set_attribute("weightingPower", 0.0)
        ...
        if momentum_record:
            ...
            particles["momentum"].set_attribute("weightingPower", 1.0)
        ...
        if "w" in species_data:
            ...
            particles["weighting"][SCALAR].set_attribute("weightingPower", 1.0)
        if "r_to_x" in species_data:
            ...
            particles["r_to_x"][SCALAR].set_attribute("macroWeighted", np.uint32(0))
            particles["r_to_x"][SCALAR].set_attribute("weightingPower", 1.0)
        if "id" in species_data:
            ...
            particles["id"][SCALAR].set_attribute("macroWeighted", np.uint32(0))
            particles["id"][SCALAR].set_attribute("weightingPower", 1.0)
        q = species_data["q"]
        m = species_data["m"]
        ...
        particles["charge"][SCALAR].set_attribute("weightingPower", 1.0)
        particles["mass"][SCALAR].set_attribute("weightingPower", 1.0)

    def _write_fields(self, it, wf_data):
        """Write wakefield diagnostics."""
        # Set common field attributes.
        it.meshes.set_attribute("fieldSolver", wf_data["field_solver"])
        it.meshes.set_attribute("fieldSolverParams", wf_data["field_solver_params"])
        it.meshes.set_attribute("fieldBoundary", wf_data["field_boundary"])
        it.meshes.set_attribute("fieldBoundaryParams", wf_data["field_boundary_params"])
        it.meshes.set_attribute("particleBoundary", wf_data["particle_boundary"])
        it.meshes.set_attribute(
            "particleBoundaryParams", wf_data["particle_boundary_params"]
        )
        it.meshes.set_attribute("currentSmoothing", wf_data["current_smoothing"])
        it.meshes.set_attribute("chargeCorrection", wf_data["charge_correction"])
        ...
        for field in wf_data["fields"]:
            ...
            if "comps" in wf_data[field]:
                ...
                fld[SCALAR].set_attribute("position", wf_data[field]["position"])
    ...
    def check_species_names(
        self, species_list: Optional[List[ParticleBunch]] = []
    ) -> None:
        ...
        for species in species_list:
            ...
            if name not in sp_names:
                ...
                raise ValueError("Several species share same name {}.".format(name))
